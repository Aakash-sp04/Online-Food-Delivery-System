            ...
            <div className='container'>
                <div className='row'>
                    ...
                    {orderData !== {} ? Array(orderData).map(data => {
                        return (
                            data.orderData ?    //if exist data.orderData
                                data.orderData.order_data.slice(0).reverse().map((item) => {    //fetch data in reverse to get latest data at first
                                    return (
                                        item.map((arrayData) => {
                                            return (
                                                <div  >
                                                    {arrayData.Order_date ? <div className='m-auto mt-5'>
                                                        ...
                                                        <div className="d-flex justify-content-center" > {/*Card as it is*/}
                                                            <div className="card mt-3" style={{ width: "20rem", maxHeight: "360px" }}>
                                                                <div className="card-body">
                                                                    <h5 className="card-title">FoodItem : {arrayData.name}</h5>
                                                                    <div className='container' style={{ height: "50px" }}>
                                                                        <span className='m-1'>Quantity : {arrayData.qty}</span>
                                                                        <span className='m-1'>{arrayData.size}</span>
                                                                        <div className='h-50 w-20 fs-5' >
